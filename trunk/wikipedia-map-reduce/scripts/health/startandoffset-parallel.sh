...
while ($index <= $end)
	...
	# Set up a temporary output directory to avoid concurrency problems with using the same output files
	set tmpdir = $outputdir/tmp-$fileindex
	mkdir $tmpdir
	mkdir $tmpdir/startYears

	cp $outputdir/startAndOffsetEdits.py $tmpdir

	echo `date` Forking worker to process $filename

	csh startandoffset-worker.sh $inputdir/$filename $tmpdir &

	@ index = $index + 1

end

# this removed it before workers can use it rm $outputdir/startAndOffsetEdits.py		# Get rid of the script when it's done to clean up
...
echo `date` Finished spawning workers to process files $nametemplate from $begin to $end with input from $inputdir, output to $outputdir, and separator script from $tooldir.
